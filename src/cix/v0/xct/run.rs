...
async fn run_xct(chan: &mut TCPStream) -> ci::Result<()> {
    const MSG_SIZE_LIMIT: usize = 128usize * 1024usize;
    let mut buf = [0u8; MSG_SIZE_LIMIT];
    loop {
        let mut lenbuf = [0u8; 4];
        chan.read_exact(&mut lenbuf).await?;
        let msglen = u32::from_be_bytes(lenbuf) as usize;
        if msglen > MSG_SIZE_LIMIT { return Err(Box::new(TooBigMessage)) }
